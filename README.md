<h1 align="center">Hi 👋, I'm Vishal Kumar</h1>
<h3 align="center">Electronics & Communication Engineering Student | Full-Stack Developer | Flutter App Developer | DSA </h3>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=vishal-IIT28&label=Profile%20views&color=0e75b6&style=flat" />
</p>

---

## 🚀 About Me

🎓 B.Tech in **Electronics and Communication Engineering**
🏫 Indian Institute of Technology (ISM), Dhanbad

💻 Passionate about **Full-Stack Development, Mobile Apps, and Systems Engineering**

* 🔭 Currently building **JellyChat – Real Time Messaging App**
* 🌱 Learning **Advanced Data Structures and Backend Systems**
* ☁️ Exploring **Cloud Deployment and Scalable Architectures**
* ⚡ Love solving problems on **LeetCode & Codeforces**

---

## 🧠 Technical Skills

### Languages

C • C++ • Java • JavaScript • Dart • Verilog HDL

### Backend Development

Node.js • Express.js • REST APIs • Socket.io

### Mobile Development

Flutter • Dart

### Databases

MongoDB • MySQL

### Tools & Cloud

AWS S3 • ASW EC2 • Git • GitHub • Postman • VS Code • Vivado • Wokwi

### Core CS

Data Structures • Algorithms • OOP • Operating Systems • Computer Networks

---

# 🛠️ Projects

### 🏠 GharForU (Accommodation Platform) (Website + Lite App)

Tech Stack: Node.js • Express • MongoDB • AWS S3 • Google Maps API

• Built a **location-based room discovery platform**
• Supports **25+ listings and 100+ users**
• Implemented **JWT authentication and role-based access**
• Used **MongoDB geospatial queries for map search**

---

### 💬 JellyChat (Real-Time Messaging App)

Tech Stack: Flutter • Node.js • Express • Socket.io • MySQL

• Real-time messaging application
• Cross-platform Flutter UI
• Backend built with scalable WebSocket architecture
• Designed relational chat database schema

---

### 🤖 RC Defensive Bot

Tech Stack: Arduino IDE • Wokwi

• Built a combat robotics **defensive bot from scratch**
• Implemented motor control and remote operation system
• Achieved **2nd Place in Roborumble’24**

---

### 🚜 Manual Armed Rover Bot

Tech Stack: Arduino IDE • Embedded Systems

• Designed rover with **mechanical arms for robotics tasks**
• Implemented servo motor control and wiring integration
• Achieved **3rd Place at Concetto’24 robotics event**

---

# 📊 GitHub Stats

<p align="center">

<img src="https://github-readme-stats.vercel.app/api?username=vishal-IIT28&show_icons=true&theme=tokyonight"/>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=vishal-IIT28&theme=tokyonight"/>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=vishal-IIT28&layout=compact&theme=tokyonight"/>

</p>

---

# 🏆 Achievements

🏅 NMMS Scholarship (2019)

🏅 Selected for **Abhayanand Super-30**

🏅 **OBC Rank 1713 – JEE Advanced**

🏅 **OBC Rank 7714 – JEE Main**

🥈 **2nd Place – RC Defensive Bot (Roborumble’24)**

🥉 **3rd Place – Manual Armed Rover Bot (Concetto’24)**

---

# 🌐 Coding Profiles

💻 LeetCode
https://leetcode.com/u/vishal_Ignite/

💻 Codeforces
https://codeforces.com/profile/vishal_Ignite

---

# 📬 Connect With Me

**Email**
[vishal.kr38897@gmail.com](mailto:vishal.kr38897@gmail.com)

**LinkedIn**
https://www.linkedin.com/in/vishal-kumar-423311320/

**Instagram**
https://www.instagram.com/me.vishal_kr/

**GitHub**
https://github.com/vishal-IIT28




---

⭐ From Vishal Kumar
